...
router.get('/linkedin/callback', async(req, res) => {
    ...
    const res_token=await axios.post(access_token_url).then((res)=>{
    ...
    if(access_token){
    const res_user_info=await axios.get(user_info_url,{
        headers: {
   ...
   if(user_info){
    ...
    connection.query('select * from undefined.users where LinkedinID=?', [LinkedinID], function (err, results, fields) {
        if (err) throw err;
        ...
        if(results.length==0){
            ...
            connection.query('INSERT INTO undefined.users (LinkedinID,name,email,picture) VALUES (?, ?,?,?)', [LinkedinID,name,email,picture], function (err, results, fields) {
                if (err) throw err;
                console.log('Inserted into api_data table.');
            }
            );

            connection.query('delete from undefined.prompt where LinkedinID=?', [LinkedinID], function (err, results, fields) {
                if (err) throw err;
                console.log('Deleted from prompts table.');
            }
            );

            connection.query('delete from undefined.postprompt where LinkedinID=?', [LinkedinID], function (err, results, fields) {
                if (err) throw err;
            ...
            DPFComment.forEach(element => {
                connection.query('INSERT INTO undefined.prompt (tone,prompt,LinkedinID) VALUES (?, ?,?)', [element.tone,element.prompt,LinkedinID], function (err, results, fields) {
                    if (err) throw err;
            ...
            DPprompt.forEach(element => {
                connection.query('INSERT INTO undefined.postprompt (Example,prompt,LinkedinID) VALUES (?, ?,?)', [element.example,element.prompt,LinkedinID], function (err, results, fields) {
                    if (err) throw err;
...
router.get("/linkedin/login/success", (req, res) => {
    ...
    try{
    connection.query('select * from undefined.users where LinkedinID=?', [user_info1.LinkedinID], function (err, results, fields) {
        if (err) throw err;
        if(results.length==0){
